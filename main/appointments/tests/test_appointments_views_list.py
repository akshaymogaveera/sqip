...
class TestListUserAppointments:
    ...
    def test_list_user_appointments_scheduled(self):
        ...
        assert (
            response.json().get("count") == 1
        )  # Should return only the scheduled appointment
    ...
    def test_list_user_appointments_no_appointments(self):
        ...
        assert response.status_code == status.HTTP_200_OK
        assert (
            response.json().get("count") == 0
        )  # No appointments should return an empty list
...
class TestListScheduledAppointments:
    ...
    def test_list_scheduled_staff_access(self):
        ...
        assert (
            len(response.data["results"]) >= 1
        )  # Staff should see all scheduled appointments
    ...
    def test_list_scheduled_regular_user_access(self):
        ...
        )  # Regular user shouldn't see other users' appointments

    def test_list_scheduled_filter_by_category(self):
        """Test filtering by category_id in list_scheduled."""
        url = (
            f"{reverse('appointments-list-scheduled')}?category_id={self.category1.id}"
        )
        response = self.client.get(url)
        assert response.status_code == status.HTTP_200_OK
        assert (
            len(response.data["results"]) == 1
        )  # Filter should return only matching category appointments
    ...
    def test_list_scheduled_invalid_category_id(self):
        ...
        assert response.status_code == 400
        assert response.json() == {
            "category_id": ["One or more category IDs are invalid."]
        }
...
class TestListUnscheduledAppointments:
    ...
    def test_list_unscheduled_filter_by_category(self):
        ...
        assert (
            len(response.data["results"]) == 1
        )  # Should return only matching category unscheduled appointments
    ...
    def test_list_unscheduled_filter_by_multiple_category_ids(self):
        ...
        assert (
            len(response.data["results"]) == 2
        )  # Should return both unscheduled appointments
